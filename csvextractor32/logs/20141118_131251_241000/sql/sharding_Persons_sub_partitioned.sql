set pagesize 0 feedback off TIMING OFF
	...
	FROM (SELECT DISTINCT grp,PARTITION_NAME,
				   ...
				   FROM (SELECT   segment_name, relative_fno, block_id,PARTITION_NAME,
							 ...
							 FROM dba_extents
							WHERE segment_name =
											 UPPER ('Persons_sub_partitioned')
							  AND owner = 'SCOTT'
							  AND UPPER(PARTITION_NAME)=UPPER('SALES_APR2000_SP1')
						 ORDER BY block_id)),
	   (SELECT data_object_id, SUBOBJECT_NAME
		  FROM all_objects
		 WHERE object_name = UPPER ('Persons_sub_partitioned') and owner='SCOTT' and data_object_id is not null  AND UPPER(SUBOBJECT_NAME) = UPPER('SALES_APR2000_SP1'))
